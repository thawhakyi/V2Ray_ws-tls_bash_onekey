...
web_camouflage () {
    ...
    https://github.com/Ollie-lee/natours.git
...
nginx_install () {
    ...
    -zxvf openssl- tar " $ openssl_version " .tar.gz
    ...
    ./configure --prefix= " ${nginx_dir} " \
        ...
        = .. OpenSSL---with / openssl- " $ openssl_version "
    judge " compile check "
    make -j " ${THREAD} "  && make install
...
port_exist_check () {
    if [[ 0 -eq  $( lsof -i: " $1 "  | grep -i -c " listen " ) ]] ;  then
        ...
        echo -e " ${Error}  ${RedBG} detects that port $1 is occupied, the following is the occupation information of port $1 ${Font} "
        lsof -i: " $1 "
...
acme () {
    if  " $HOME " /.acme.sh/acme.sh --issue -d " ${domain} " --standalone -k ec-256 --force --test ;  then
        echo -e " ${OK}  ${GreenBG} SSL certificate test issuance is successful, and the official issuance of ${Font} is started .
...
enable_process_systemd () {
    ...
    judge " Set v2ray to start automatically after boot "
    if [[ " $shell_mode "  !=  " h2 " ]] ;  then
